...
- The old logo files (`/logo.svg`, `/logo-icon.svg`) are 840 KB fake SVGs with a baked-in white background — RULE: the site logo is `/logo-mark.png` (transparent, 384px, ~49 KB). Never reference logo.svg/logo-icon.svg in code; no blend workarounds needed. Favicon is `app/icon.png` from the same mark.
...
- Axe flagged serious contrast failures sitewide — RULE: `text-zinc-400`/`text-zinc-500` are BANNED as text colors on light backgrounds (2.6:1); secondary text on cream/white is always `text-slate`. On the ink footer/bands, use `text-zinc-400` or lighter (passes on dark). The wordmark "Sniff" uses `text-green-deep` on light surfaces and bright `text-green-accent` only on dark (bright green on cream is 1.9:1 and fails even large-text AA).
...
- Astrology quiz zodiac symbols — DOCUMENTED EXCEPTION to "no emojis as icons": the sign glyphs (♈–♓, Unicode U+2648–U+2653) are kept INTENTIONALLY, not drawn as custom SVGs. Rationale: zodiac glyphs are a recognized symbolic alphabet (unlike the abstract mood/season concepts that genuinely need drawn icons), so custom icons add cost without clarity. They are pinned to text presentation with the U+FE0E variation selector (`'♈︎'`) because by default these characters render as COLOR emoji on iOS/Android — FE0E forces the monochrome symbol — and shown in a Green Wash icon tile tinted `text-green-deep`, matching the other quizzes' icon-tile treatment. Do NOT "fix" these into custom SVGs and do NOT flag them as an emoji-rule violation; this is a deliberate, tested decision (Joseph, 2026-07-20).
