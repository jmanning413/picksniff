...
- Guests get 10 results / accounts 20 (`limit` in `app/results/page.js` and `/api/quiz/match`). The Signature Scent Quiz is **exactly 4 steps for everyone** (gender, tier, vibe, accords) — never add quiz steps; account perks are the result limit, wishlist, and collection only.
...
- **PickSniff is 100% free. NEVER add premium tiers, subscriptions, paid features, or paywalls. Stripe is for optional donations ONLY and donations never unlock features. Free account creation is the only gate that exists.** Premium code was fully removed June 2026; `/premium` 301s to `/support` via next.config.mjs. Deprecated `is_premium`/`premium_expires_at`/`stripe_customer_id`/`profile_border_color` columns remain in the live DB unread; drop them in the next schema migration.
