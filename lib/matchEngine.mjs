...
export function rankFragrances(pool, { tier, vibe, accords = [], notes = [], limit = 20, tierLoosened = false }) {
  ...
  // De-duplicate: the catalog has some repeated entries (same product split
  // across tiers, or straight extraction dupes). Never show the same
  // brand+name+concentration twice; keep the highest-scored instance.
  const seen = new Set()
  const deduped = []
  for (const f of scored) {
    const key = `${normalize(f.brand)}|${normalize(f.name)}|${normalize(f.concentration)}`
  ...
  const guaranteed = applyAccordGuarantee(deduped, accords)
